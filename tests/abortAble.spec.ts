...
describe('utils', () => {
  describe('abortAble', () => {
    ...
    it('simple', () => {
      ...
      return Promise.resolve(a.then((r) => expect(r).toBe(1)));
    });
    it('resolve simple', () => {
    ...
    it('simple abort', () => {
      const a = abortAble(Promise.resolve(1));
      a.abort();
      return Promise.resolve(a.then((r) => expect(r).toBe(ABORTED)));
    });
    it('resolve abort', () => {
      const a = abortAble(resolveIn(10, 1));
      a.abort();
    ...
    it('resolve chain', () => {
      ...
      return Promise.resolve(b.then((r) => expect(r).toBe(10)));
    });
    it('resolve chain abort', () => {
      const a = abortAble(resolveIn(10, 1));
      a.abort();
      const b = a.then((v) => (typeof v === 'symbol' ? v : v * 10));
      return Promise.resolve(b.then((r) => expect(r).toBe(ABORTED)));
    });
